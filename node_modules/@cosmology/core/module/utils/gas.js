...
export const gasEstimation = async (denom, stargateClient, address, msgs, memo, modifier) => {
  ...
  const simulate = async (address, msgs, memo, modifier = 1.5) => {
    const estimate = await stargateClient.simulate(address, msgs, memo); // console.log({ estimate });
  ...
  const getGasPrice = async (address, msgs, memo, modifier) => {
    ...
    try {
      ...
      return fee; //   const feeAmount = Number(fee.amount[0].amount);
      //   return feeAmount;
    } catch (error) {
      console.log(error);
    }
