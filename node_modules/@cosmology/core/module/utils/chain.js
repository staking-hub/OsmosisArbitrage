...
export const getCosmosAssetInfo = symbol => assets.find(a => !!a.assets.find(i => i.symbol === symbol && i.type_asset !== 'ics20'));
export const getOsmosisAssetInfo = symbol => osmosisAssets.find(a => a.symbol === symbol && a.type_asset !== 'ics20');
export const getCosmosAssetInfoByDenom = denom => assets.find(a => !!a.assets.find(asset => !!asset.denom_units.find(unit => unit.denom === denom) && asset.type_asset !== 'ics20'));
export const getOsmosisAssetInfoByDenom = denom => osmosisAssets.find(a => !!a.denom_units.find(unit => unit.denom === denom) && a.type_asset !== 'ics20');
...
export const getBaseAndDisplayUnits = symbol => {
  ...
  const display = coinInfo.denom_units.find(d => {
    var _d$aliases2;
...
export const getBaseAndDisplayUnitsByDenom = denom => {
  ...
  if (!coinInfo) {
    ...
    coinInfo = chainInfo.assets.find(asset => asset.base === denom || asset.display === denom);
...
export const baseUnitsToDisplayUnitsByDenom = (denom, amount) => {
  const {
    display
  } = getBaseAndDisplayUnitsByDenom(denom);
  const a = new IntPretty(new Dec(amount));
  return a.moveDecimalPointLeft(display.exponent).maxDecimals(16).locale(false).toString();
};
export const getChain = async ({
  ...
  const chainFromAssets = assets.find(({
    assets
  }) => {
  ...
  const chain = chains.find(({
    chain_name
  }) => chain_name == chainFromAssets.chain_name);
  return chain;
};